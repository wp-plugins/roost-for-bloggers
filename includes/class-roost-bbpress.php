...
class Roost_bbPress {
    ...
    public function __construct() {
        add_action( 'wp_enqueue_scripts', array( $this, 'load_roost_bbp_scripts' ), 1 );

        add_action( 'wp_ajax_roost_bbp_subscribe', array( $this, 'roost_bbp_ajax_subscription' ) );
        add_action( 'wp_ajax_nopriv_roost_bbp_subscribe', array( $this, 'roost_bbp_ajax_subscription' ) );
        add_action( 'wp_ajax_roost_bbp_unsubscribe', array( $this, 'roost_bbp_ajax_subscription' ) );
        add_action( 'wp_ajax_nopriv_roost_bbp_unsubscribe', array( $this, 'roost_bbp_ajax_subscription' ) );

        add_action( 'bbp_theme_before_reply_form_submit_wrapper', array( $this, 'roost_bbp_reply_subscription' ) );
        add_action( 'bbp_theme_before_topic_form_submit_wrapper', array( $this, 'roost_bbp_reply_subscription' ) );
        add_action( 'bbp_template_before_single_topic', array( $this, 'roost_bbp_topic_subscription' ) );        
        add_action( 'bbp_template_before_single_forum', array( $this, 'roost_bbp_forum_subscription' ) );
        add_action( 'bbp_get_request', array( $this, 'roost_bbp_subscriptions_handler' ), 1 );
        add_action( 'bbp_new_reply', array( $this, 'roost_bbp_subscriptions_handler' ), 1 );
        add_action( 'bbp_edit_reply', array( $this, 'roost_bbp_subscriptions_handler' ), 1 );
        add_action( 'bbp_new_topic', array( $this, 'roost_bbp_subscriptions_handler' ), 1 );
        add_action( 'bbp_edit_topic', array( $this, 'roost_bbp_subscriptions_handler' ), 1 );
        
        add_action( 'bbp_new_reply', array( $this, 'roost_bbp_notify_subscribers' ), 11, 5 );
        add_action( 'bbp_new_topic', array( $this, 'roost_bbp_notify_subscribers' ), 11, 5 );

        add_action( 'bbp_delete_reply', array( $this, 'roost_bbp_remove_all_subscriptions' ) );
        add_action( 'bbp_delete_topic', array( $this, 'roost_bbp_remove_all_subscriptions' ) );
        add_action( 'bbp_delete_forum', array( $this, 'roost_bbp_remove_all_subscriptions' ) );
    }

    public function load_roost_bbp_scripts() {
        wp_enqueue_script( 'roostbbpjs', ROOST_URL . 'layout/js/roostbbp.js', array('jquery'), Roost::$roost_version, false );
	}
    
    public function roost_bbp_reply_subscription( $post ) {
        global $post;
                ...
                setTimeout(function(){
                    if(window.roostEnabled){
                        ...
                        ?>
                                ...
                                if (typeof registrations[window.roostToken] !== 'undefined') {
                                    if(registrations[window.roostToken] === true) {
    ...
    public function roost_bbp_topic_subscription( $post ) {
        global $post;
        $post_id = $post->ID;
        ...
        <script>
            jQuery(document).ready(function($) {
                ...
                subscribeLink.on('click', function(e){
                    ...
                    if(subscribeLink.data('action') === 'roost_bbp_subscribe'){
                        subscribeLink.text('Unsubscribe from Push Notifications');
                        subscribeLink.data('action', 'roost_bbp_unsubscribe');
                    } else {
                        subscribeLink.text('Subscribe with Push Notifications');
    ...
    function roost_bbp_ajax_subscription() {
        $post_id = $_POST['postID'];
        self::roost_bbp_subscriptions_handler( $post_id );
        die();
    }
    
    public function roost_bbp_forum_subscription( $post ) {
        global $post;
        $post_id = $post->ID;
    ...
    public function roost_bbp_subscriptions_handler( $post_id ) {
    ...
    public function roost_bbp_remove_all_subscriptions( $action ) {
        delete_post_meta( $action, 'roost_bbp_subscription' );
    }
    
    public function roost_bbp_notify_subscribers( $reply_id, $topic_id, $forum_id, $anonymous_data, $reply_author = 0 ) {
